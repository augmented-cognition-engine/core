...
`degraded/effect_unknown` instead of being silently retried. The host registers adapters by exact
artifact identity through `BoundedActionAdapterRegistry`; it performs no entry-point discovery,
dynamic import, prefix matching, or version fallback. The supported SurrealDB composition path and
...
the [L1 evidence gate](evidence/l1-foresight-impact-evidence.md) records the bounded executable-workload
benefit result. I3 and L1 are passed within those frozen scopes; human, external-product, and general
real-world benefit remain outside the supported claim.
