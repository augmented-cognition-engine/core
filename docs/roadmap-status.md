...
| E1 | Next | not ready | The [`extension-invocation-v1` evidence](extension-invocation-contract.md) freezes the experimental durable bridge; [Phase 5](phase-5-extension-sdk-acceptance.md) records SDK/current-version conformance; [Phase 6](phase-6-extension-product-experience-acceptance.md) accepts the receipt/recovery product experience; and [Phase 7](phase-7-extension-invocation-stabilization-review.md) conditionally passes the current-version stabilization review. Stable promotion remains blocked on N-1 package skew, trusted-process isolation/security review, distributed recovery/effect semantics, and complete operability/resource evidence. |
