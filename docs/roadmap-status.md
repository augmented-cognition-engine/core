...
|---|---|---|---|
| R1 | Now | ready | Execute the full clean-user journey on macOS and Linux with normal dependencies and documented services |
| R2 | Now | not ready | Requires R1 findings; then cut 0.1.1 with focused PyPI copy, repaired absolute links, and release-workflow maintenance |
| R3 | Now | ready | Verify supported provider routes, authentication, `ace doctor`, actionable failures, and degraded behavior |
| R4 | Now | not ready | Requires R1 and R3 to pass before publishing the public golden-path demonstration |
...
1. **Required spine — R1:** clean-user installation and onboarding audit.
2. **Safe parallel — R3:** provider, authentication, diagnostics, and degraded-state validation.
3. **Queued — R2:** 0.1.1 packaging and documentation polish after R1 records findings.
4. **Do not start yet — R4:** public golden-path demonstration until R1 and R3 pass.
