...
| E1 | Next | not ready | The [`extension-invocation-v1` evidence](extension-invocation-contract.md) freezes the experimental durable bridge, and [Phase 5 SDK acceptance](phase-5-extension-sdk-acceptance.md) records independent reference/scaffold conformance plus the tested current-version matrix. The task-action surface remains experimental; broader cross-release compatibility and Phases 6–7 are not accepted. |
