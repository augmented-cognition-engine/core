...
![published version 1.2.2](https://img.shields.io/badge/published-1.2.2-blue)
...
The current published stable package and public-index install is `ace-core==1.2.2`, the bounded
...
The public-index package, GitHub Release, and this source tree now identify `ace-core==1.2.2`.
...
**ACE 1.2 is stable for the documented single-user, single-node topology; 1.2.2 is the recommended
release.**

| Status | Product boundary |
|---|---|
| **Stable 1.2.2** | The bounded personal Intelligence OS plus Code Intelligence, plus Personal Intelligence as a Solution Bundle: read-only local knowledge (Markdown/Obsidian, PDF, CSV, JSON) with cited Briefs, append-only change revisions, grounded Ask with claim-bound corrections, truthful export/deletion, and exact bundle activation over the shared substrate |
