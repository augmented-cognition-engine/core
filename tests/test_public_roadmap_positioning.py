...
# The current-release assertions preserve published 0.8.2 while admitting the bounded 0.8.3
# compatibility candidate. Historical 0.4.1 GI2,
...
def test_current_release_and_passed_milestone_are_not_conflated() -> None:
    assert "latest published release is [`ace-core` 0.8.2]" in ROADMAP
    assert "passed 0.8.2 patch adds the Atrium Intelligence Catalog" in ROADMAP
    assert "current 0.8.3 compatibility release candidate" in ROADMAP
    ...
    assert "0.8.3 is a release candidate over the published 0.8.2 developer preview" in " ".join(readme.split())
