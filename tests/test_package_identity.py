...
def test_distribution_import_cli_and_version_identities() -> None:
    ...
    assert project["version"] == ace.__version__ == ace_mcp_client.__version__ == VERSION == "0.8.2"
...
def test_docker_image_includes_public_cli_package() -> None:
    ...
    assert "ARG ACE_VERSION=0.8.2" in dockerfile
    ...
    assert compose.count('ACE_VERSION: "0.8.2"') == 3
    assert compose.count('org.opencontainers.image.version: "0.8.2"') == 2
