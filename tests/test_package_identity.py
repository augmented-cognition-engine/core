...
def test_distribution_import_cli_and_version_identities() -> None:
    ...
    assert project["version"] == ace.__version__ == ace_mcp_client.__version__ == VERSION == "1.1.0"
...
def test_release_workflow_defaults_to_and_guards_current_version() -> None:
    ...
    assert "default: v1.1.0" in workflow
...
def test_docker_image_includes_public_cli_package() -> None:
    ...
    assert "ARG ACE_VERSION=1.1.0" in dockerfile
    ...
    assert compose.count('ACE_VERSION: "1.1.0"') == 3
    assert compose.count('org.opencontainers.image.version: "1.1.0"') == 2
...
def test_lock_tracks_the_distribution_identity() -> None:
    ...
    assert [package["version"] for package in local] == ["1.1.0"]
