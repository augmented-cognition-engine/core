...
def test_distribution_import_cli_and_version_identities() -> None:
    ...
    assert project["version"] == ace.__version__ == ace_mcp_client.__version__ == VERSION == "0.4.1"
...
def test_release_workflow_defaults_to_and_guards_current_version() -> None:
    ...
    assert "default: v0.4.1" in workflow
...
def test_docker_image_includes_public_cli_package() -> None:
    ...
    assert "ARG ACE_VERSION=0.4.1" in dockerfile
    ...
    assert compose.count('ACE_VERSION: "0.4.1"') == 3
    assert compose.count('org.opencontainers.image.version: "0.4.1"') == 2
