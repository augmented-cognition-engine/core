...
def test_distribution_import_cli_and_version_identities() -> None:
    ...
    assert project["version"] == ace.__version__ == ace_mcp_client.__version__ == VERSION == "1.0.2"
...
def test_docker_image_includes_public_cli_package() -> None:
    ...
    assert "ARG ACE_VERSION=1.0.2" in dockerfile
    ...
    assert compose.count('ACE_VERSION: "1.0.2"') == 3
    assert compose.count('org.opencontainers.image.version: "1.0.2"') == 2
