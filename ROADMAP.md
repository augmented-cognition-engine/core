...
**ACE, the Intelligence Builder. Build intelligence, not infrastructure.** ACE is an open,
provider-neutral Intelligence Operating System for turning authorized sources into living,
source-grounded briefings, monitors, decisions, and improving institutional knowledge. Models may
provide inference inside the loop. ACE owns the durable state, composition, lifecycle, authority,
provenance, and outcome loop around it.
...
The north-star builder journey is:

```text
install → Connect → Map a concept model → Watch what matters → Brief with citations → Activate
→ keep updating → improve relevance through feedback
```

The outcome must arrive before the architecture lesson. A builder should not need to understand
Core, Intelligence, Domain Packs, compiler mechanics, or hand-authored JSON to reach first value.
Those boundaries remain essential implementation and trust guarantees underneath a guided product
experience.
...
> Can a builder connect authorized sources and receive useful, source-grounded intelligence that
> stays current and improves with feedback—without building the underlying infrastructure,
> surrendering human authority, or learning ACE's internal layers first?
...
Intelligence 1.2 is **passed** for nine of its ten journey steps against published 1.2.3 artifacts,
with continuous update disclosed as absent and sequenced to 1.3. Intelligence Operations and Safe
Evolution 1.3 is now **Now**.
...
The [0.8.3 GitHub Release](https://github.com/augmented-cognition-engine/core/releases/tag/v0.8.3)
and public [`ace-core==0.8.3`](https://pypi.org/project/ace-core/0.8.3/) package close the 0.8
compatibility line. The 0.8.1 foundation binds live onboarding state into Atrium, defines
...
| 1.1 | Code Intelligence | **Passed** | Public Core 1.1.0 reproduces the repository-to-reasoning graph, bounded downstream impact, untrusted-repository handoff, the governed improvement-loop contracts, delegated review/activation, SurrealDB 3.2/v179 recovery, exact eleven-tool surface, and checkout-free public-index install. The public release delivers bounded read-only reasoning and handoff; active loop execution is sequenced later (linked repair in 1.3, future-work learning in 1.6) | [#194](https://github.com/augmented-cognition-engine/core/issues/194) |
| 1.2 | Personal Intelligence | **Passed** | A durable personal intelligence environment beginning with read-only local Markdown/Obsidian, PDF, CSV, and JSON sources; at most one remote knowledge source follows only after the local journey passes. The bundle binds exact independently released adapter versions without owning them or requiring a universal connector catalog. Public `ace-core` 1.2.3 artifacts reproduce install, choose, connect, inventory, first cited Brief, ask, correct, restart, and own; continuous update is sequenced to 1.3 | [#195](https://github.com/augmented-cognition-engine/core/issues/195) |
| 1.3 | Intelligence Operations and Safe Evolution | **Now** | Version discovery, Atrium update experience, compatibility-aware upgrade plans, migration receipts, backup, rollback, health, and recovery so upgrades improve the experience without surprising users. Continuous update for the Personal journey lands here: 1.2 delivers cited Briefs that are rebuilt when the owner asks, and the watched-source path that keeps them current is operations work | [#196](https://github.com/augmented-cognition-engine/core/issues/196) |
...
### 0.7.0 — Intelligence Builder Foundation

Public promise: a builder can connect authorized sources and reach a useful first briefing and
monitoring loop without building intelligence infrastructure or understanding ACE's internal
layers. Third parties can still build, test, distribute, and operate the portable Domain Packs,
connectors, and trusted extensions underneath that experience.

- **0.7A — stable contract/compiler/conformance:** the PR #100 candidate freezes machine-readable
  generated Domain Pack schemas, compatibility, deterministic conformance, activation refusal, and
  lifecycle policy.
- **0.7B — Connect:** the Connection Agent proposes supported source options, exact permission and
  scope, provider-free fixture profiles, and resumable blocked/retry state without owning transport
  or credentials. Its bounded implementation and evidence gates are frozen in the
...
milestone after the cumulative A–E journey is accepted.

Release gate: one clean builder journey reaches a source-grounded first Brief without hand-authored
pack material or required knowledge of ACE internals; independent packs, connectors, and trusted
extensions pass their applicable public compatibility and conformance checks; requested source
scope and authority are visible before activation; and failures remain isolated and diagnosable.

### 0.8.0 — Intelligence OS Realignment

Public promise: ACE presents one coherent Intelligence Operating System. Governed data enters,
evidence-grounded intelligence is derived, people and machines consume it downstream, and
attributable outcomes return through governed feedback. Reasoning is a governed component of that
lifecycle rather than the product category. Atrium is its permission-aware intelligence dashboard
and human control plane.

The bounded delivery sequence is:

- **0.8A — architecture and compatibility:** freeze the canonical lifecycle, ownership map,
  compatibility seams, work packets, and golden acceptance journey.
- **0.8B — runtime realignment:** enforce Core, Intelligence, Domain Pack, adapter/plugin, and
  application ownership in code while preserving supported contracts through declared migration.
- **0.8C — unified intelligence resources:** expose one public resource plane for sources, entity
  state, Observations, Signals, Shifts, Cases, Briefs, monitors, subscriptions, decisions, actions,
...
- **0.8F — release acceptance:** pass compatibility, migration, restart, installed-artifact,
  accessibility, entitlement, degraded-state, and two-domain publication gates.

Canonical current architecture and product surfaces replace the octopus, arm-growth, MAKE/SHIP,
partner-team, committee-first, product-manager-toolbox, and Living Product Graph system-category
narratives. Historical evidence and supported compatibility identities are not rewritten.

The complete packet, compatibility map, landing order, stop conditions, and signature demonstration
are frozen in the
[Intelligence OS Realignment work packet](docs/design/intelligence-os-realignment-v0.8.0-work-packet-v1.md).

Release gate: World and Market complete the same governed evidence-to-outcome lifecycle without a
domain branch in Core or Intelligence; Atrium and a machine interface consume the same public
durable intelligence; supported compatibility and migrations are tested; and accessibility,
...
Outcome state: **passed in the cumulative 1.0 candidate.** The product journey below is complete;
its public-artifact reproduction is performed as part of the 1.0 release gate rather than as a
separate 0.9 publication.
...
The 1.1 improvement model freezes three separately governed loops as contracts. The public 1.1
release delivers bounded read-only reasoning and handoff; active execution of these loops is
sequenced across later releases as noted below.
...
The reference acceptance program is **ACE Builds ACE**. By owner decision (2026-08-17) it is
executed in ACE 1.2 with bounded 1.2 implementation Decisions as its subject, using shipped 1.1
capabilities only; it was not part of the public 1.1 release. The program: freeze a real roadmap
Decision, compare
...
bundles; the Personal Solution Bundle binds exact adapter versions without owning their
implementations, and this roadmap does not promise a universal connector catalog. The public
contract lives in [issue #195](https://github.com/augmented-cognition-engine/core/issues/195).
...
- **1.3 Intelligence Operations and Safe Evolution:** version discovery, Atrium update flows,
  compatibility-aware upgrades, migration receipts, backup, rollback, health, and recovery — and
  continuous update for the Personal journey, whose watched-source path is operations work rather than
  part of the 1.2 build path.
...
## Intelligence Operating System architecture sequence
...
bounded context package to a coding agent. The current 1.x narrative additionally requires
