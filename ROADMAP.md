...
| R3 | candidate | Validate provider setup, authentication, diagnostics, and degraded behavior | Matrix, deterministic degraded-state coverage, one live GPT route, and pre-update branch CI are evidenced; an authorized live Claude route and final CI against current main remain required |
