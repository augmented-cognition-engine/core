...
| IA-R1 | candidate | Define the read-only information architecture for inspecting ACE state | [`/landscape`](docs/ia-r1-product-map.md) now composes the G1 snapshot into six operator questions with visible provenance, uncertainty, object identity, failures, and strict no-write/no-execution authority; branch/final-head/main CI and closeout remain required |
