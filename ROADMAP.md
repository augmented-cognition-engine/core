...
| 1.2 | Personal Intelligence | **Now** | A durable personal intelligence environment beginning with read-only local Markdown/Obsidian, PDF, CSV, and JSON sources; at most one remote knowledge source follows only after the local journey passes. The bundle binds exact independently released adapter versions without owning them or requiring a universal connector catalog | [#195](https://github.com/augmented-cognition-engine/core/issues/195) |
| 1.3 | Intelligence Operations and Safe Evolution | **Later** | Version discovery, Atrium update experience, compatibility-aware upgrade plans, migration receipts, backup, rollback, health, and recovery so upgrades improve the experience without surprising users. Continuous update for the Personal journey lands here: 1.2 delivers cited Briefs that are rebuilt when the owner asks, and the watched-source path that keeps them current is operations work | [#196](https://github.com/augmented-cognition-engine/core/issues/196) |
