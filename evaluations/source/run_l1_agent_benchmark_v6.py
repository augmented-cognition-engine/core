...
async def _invoke(provider: CodexCLIProvider, prompt: str, model: str) -> tuple[dict[str, Any], dict[str, Any]]:
...
def _non_exposure_receipt(
    ...
    body = {
        "contract_version": "ace.foresight.impact-agent-control-non-exposure/v6",
        "case_id": assignment["case_id"],
        "arm": assignment["arm"],
...
async def _collect(
    ...
    *,
    ...
    for index, assignment in enumerate(assignments, start=1):
        ...
        main_decision, main_metrics = await _invoke(provider, prompt, route["model"])
        ...
        if assignment["arm"] == "ace_foresight":
            ...
            shadow_decision, shadow_metrics = await _invoke(provider, shadow_prompt, route["model"])
        ...
        for pending in cluster_pending:
            ...
            if replay_seed is not None:
                ...
                outcome = {
                    ...
                    "outcome_id": f"outcome:{case_base['case_id']}:v6",
                    "observed_at": _now(),
